...
res3 <- pblapply(cl=cl,1:20, function(i){
  # Generate data
...
res4 <- pblapply(cl=cl,1:20, function(i){
  # Generate data
  data <- simulateTrigData(500)
...
save(res3, file = "result_low_power_Trigdata_H0True.RData")
save(res4, file = "result_low_power_Trigdata_H0False.RData")

#########################################################################
complete_data <- data.frame(unlist(res3))
...
ggsave("low_power_TrigData_true.eps", plot = QQplots, width = 8 , height = 5)

#################################################################################

complete_data <- data.frame(unlist(res4))
...
ggsave("low_power_TrigData_false.eps", plot = QQplots, width = 8 , height = 5)
