...
res5 <- pblapply(cl=cl,1:20, function(i){
  # Generate data
  data <- simulateNonLinearData(500)
  ...
  output <- list()
  for (j in 1:500) {
    output[[j]] <- NullGenerator(data = data, formula = Y ~ X + Z2 + Z2Sqr + Z2Cub + Z1 + Z1Sqr + Z1Cub , p = 0.80, objective = 'multi:softprob', num_class = 4)
  ...
  test <- list()
  for (j in 1:500) {
    test[[j]] <- TestGenerator(data = data, formula =  Y ~ X + Z2 + Z2Sqr + Z2Cub + Z1 + Z1Sqr + Z1Cub , p = 0.80, objective = 'multi:softprob', num_class = 4)
    cat(sprintf("Sample: %d\r", j))
    flush.console()
  }
...
res6 <- pblapply(cl=cl,1:20, function(i){
  # Generate data
  data <- simulateNonLinearData(500)
  data$Z2Sqr <- data$Z2^2
  data$Z2Cub <- data$Z2^3
  ...
  output <- list()
  for (j in 1:500) {
    output[[j]] <- NullGenerator(data = data, formula = Y ~ X + Z2 + Z2Sqr + Z2Cub , p = 0.80, objective = 'multi:softprob', num_class = 4)
    cat(sprintf("Sample: %d\r", j))
    flush.console()
  }
  NullDist <- data.frame(do.call(rbind, output))
  ...
  test <- list()
  for (j in 1:500) {
    test[[j]] <- TestGenerator(data = data, formula = Y ~ X + Z2 + Z2Sqr + Z2Cub , p = 0.80, objective = 'multi:softprob', num_class = 4)
    cat(sprintf("Sample: %d\r", j))
    flush.console()
  }
  TestDist <- data.frame(do.call(rbind, test))
...
save(res5, file = "result_low_power_nonLinear_true.RData")
save(res6, file = "result_low_power_nonLinear_false.RData")

######### PLOTTING THE RESULTS #########
complete_data <- data.frame(unlist(res5))
...
ggsave("low_power_NonLinear_true.eps", plot = QQplots, width = 8 , height = 5)

complete_data <- data.frame(unlist(res6))
...
ggsave("low_power_NonLinear_false.eps", plot = QQplots, width = 8 , height = 5)
